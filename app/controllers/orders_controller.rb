class OrdersController < ApplicationController
  ...
  def order_params
    params.require(:order_form).permit(:postal_code, :prefecture, :city, :address, :house_number, :phone_number, :token).merge(token: params[:token], user_id: current_user.id, item_id: @item.id, price: @item.price)
  end
end
